...
shape the rules do not state on their own; and the ⚖️ Decision's **Options** line, which replaces
the Recommendation.
...
comment** — see PR Comment Format. Flag them fully in the local artifact, redacting the sensitive
value itself as Redacting Sensitive Evidence describes; the published record carries only a count.
...
1. **Preserve status glyphs** — keep ❓ Open, ✅ Fixed, 🚫 Ignored and ⏸️ Deferred glyphs and their
   ...
   are the factual ones in Update findings below
...
1. **Update findings** against the current document. ✅ Fixed records a fact, so a re-review may
   ...
   - A finding whose condition is verifiably gone, including because its section was deleted, is
     ...
     ⚖️ Decision is the exception: re-reading the document cannot rule on it, so it stays as it is
...
The 🔴🟠🟡🟢 severity ladder and the ℹ️ and 💡 observations match `/local-review` (a code review
command available in project repositories), so the two kinds of review scan alike. ❓ Open, the
⚖️ Decision and the implementation groups were added here and may not exist there — do not assume a
rule in this file holds in that one, or the reverse.
...
action inline. When severity and recommendation diverge — a 🟢 Low recommended **Implement**, or a
🟠 High recommended **Defer** — that divergence is the useful signal; surface it rather than
smoothing it over.
...
A ⚖️ Decision carries its **Options** line where other findings carry a Recommendation, with each
alternative lettered and its cost stated:

```markdown
### F5 ⚖️ Decision - Whether the Quick Start covers the Docker path

- **Location** — Quick Start
- **Issue** — The Quick Start documents the native install only, while Installation presents Docker
  as an equal path. Both scopes are defensible; the document has to pick one.
- **Options** — **(a)** Document both paths: complete for every reader, but roughly doubles the
  section. **(b)** Native only, linking to Installation for Docker: stays short, but Docker users
...
misattribute — and a document review quotes Markdown far more often than a code review does. A
suggestion that quotes a `bash` block looks like this in the review file:
...
visually while preserving the original content for reference. ℹ️ and 💡 Observation findings carry
no status. When a 💡 action is taken, say so in the observation's prose; its Status cell stays `—`.
...
evidence. The exception is a ⚖️ Decision, whose parenthetical records the ruling. ⏸️ and 🚫 carry a
short reason, because each records a decision someone may later want to revisit.
...
- **⏸️** and **🚫** may be written only after the user confirms that specific finding. Never infer
  the decision from a **Defer** or **Skip** recommendation, and never prompt for it — the user
  raises it unprompted and the orchestrator records it
- A ⏸️ or 🚫 finding is reopened only by the user, and the user may reopen one at any time. It
  ...
  removed, its checklist item and summary-table Status reverted and its group heading un-checked
  if the group had been complete. Never reopen one unprompted: the rule against re-offering a
  decided finding binds the agent, not the user
...
- Leave no Status cell blank or `—` for an actionable finding; either reads as "nothing to decide
  here" and quietly closes the finding. In the Status column the em dash is reserved for ℹ️ and 💡
  observations, where no status applies; other columns use it for "not applicable" as their own
  rules define
...
The **overall assessment** — a brief summary of document quality — is not part of this section. It
is its own `## Overall Assessment` near the top of the file, where a reader meets it first; see the
skeleton under Document Header. Neither is the checklist: `## Pre-Merge Checklist` is a sibling
section, so that its groups sit one level below it. Keep the three apart rather than listing them
here: GitHub renders an ordered list interrupted by a table as separate one-item lists, and a `###`
group heading cannot be a child of a list item.
...
- **Membership.** Every finding recommended **Implement** belongs to exactly one group, as does
  ...
  **Skip** are listed after the groups under **Not recommended for this revision**, still at ❓,
  because the recommendation is advice and the user may take them anyway. Membership routes by the
  recommendation, not by where the status later lands: a grouped finding that the user then defers
  or ignores stays in its group.
...
aliases; `find -exec grep` already runs the binary directly and needs no prefix. Never give the scrub
a flag or tool that honors ignore files.
...
for part in "$work_dir"/part-*; do
  i=$((i + 1))
  [ "$(wc -c < "$part")" -le "$limit" ] \
    || { echo "one section of the review is over the limit on its own — split it by hand" >&2; exit 1; }
  label=""
  [ "$total" -eq 1 ] || label=" (part $i of $total)"
  {
    if [ "$i" -eq 1 ]; then
      ...
      echo ""
      echo "**[N findings — X fixed, Y deferred, Z ignored, W open, V observations]**"
      echo ""
    fi
    echo "<details>"
...
for comment in "$work_dir"/comment-*; do
  ...
  gh pr comment "$pr_url" --body-file "$comment" \
    || { echo "posting stopped at part $i of $total; earlier parts are already posted" >&2; exit 1; }
done
```

The comment is scrubbed twice, and the second pass is the one that matters, run over every part
before any is posted. The first covers the
...
limit whatever the text, with room for the wrapper — so a finding is never split across two
comments and a fenced block inside one is never cut open. Each part gets its own `<details>` wrapper
labeled "part 2 of 3"; only the first carries the heading and the stats line. Every part passes
Gate 2 before the first is posted, so a hit in part three cannot leave parts one and two published
with nothing to show for it. A single section longer than the limit stops the post: cut it by hand at
a paragraph boundary outside any fence, rather than letting the block split it mid-fence. If `gh`
...
separately (e.g., "24 findings — 14 fixed, 2 ignored, 2 open, 8 observations"), omitting any bucket
...
a fix. Deferred and ignored findings are off the pre-merge path but they are not resolved, so they
...
clear". That holds for every severity and for decisions alike: a 🟢 Low still ❓ Open or ⏸️ Deferred
bars "all clear" as surely as a 🔴 Critical does.
...
⚖️ Decision listed first under a heading reading `Decisions needed`. A finding the user has
already ruled on must not be re-offered: putting it back in the list reopens a decision they made.
If the user reopens one, it rejoins the list as ❓ Open. Format the list as:
...
user already made. The two views also differ in shape by design: the checklist leads with the
checkbox and status glyph so its left margin scans for what is open, while this list leads with
severity and names the location, because it asks the user to choose rather than tracking what
remains.
...
While an unanswered ⚖️ sits in a group, neither that group's identifier nor "all" selects any of
its members. Put the decision first, ask it, record the ruling and only then begin the member
edits. "all" still selects the open findings in every group that has no unanswered decision;
without that, one open ⚖️ anywhere would silently make "all" select nothing. Membership deliberately
...
If the user selects one or more findings, edit the document directly to resolve them in group order.
After each fix, record it in all four places the finding appears: strike the heading and mark it ✅
Fixed, add its Status line with the evidence, set its summary-table Status to ✅ and check its
checklist item as `- [x] ✅ F1 - …` — see Tracking Finding Status. Mark ✅ only once the edit is in
the document, and when a group's last member comes off the pre-merge path, check the group off at
its heading as the Completion rule describes. A ⚖️ answered here is recorded the same way, with the
ruling in the parenthetical.

Answering "skip" here means "not fixing any of these right now" — it is **not** a decision to mark
anything ⏸️ Deferred or 🚫 Ignored. Unselected findings stay ❓ Open in both the summary table and
the checklist.

Present the list once per review run. When the selected fixes have landed, do not present it again
with whatever is still open: those findings stay ❓ Open by the rule above, not because they are
waiting for another round. The user can ask for the list again at any time.
