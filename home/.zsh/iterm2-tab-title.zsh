...
# Control bytes are stripped here rather than at the call sites, so any future caller is covered
# too: a raw newline in a title terminates the OSC string early and prints the remainder into the
# pane, and `preexec` passes on whatever was typed — a multi-line command, or a paste.
_tab_title_emit() {
  local title=${1//[[:cntrl:]]/ }
  if [[ -n $TMUX ]]; then
